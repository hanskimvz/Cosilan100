<template>
    <nav class="navbar sidebar-sticky navbar-expand navbar-light bg-white">
      <a class="sidebar-toggle d-flex mr-2"><i class="hamburger align-self-center"></i></a>
      <div class="navbar-collapse collapse">
        <ul class="navbar-nav ml-auto">
          <li class="nav-item dropdown">
            <a class="nav-flag dropdown-toggle" href="#" id="languageDropdown" data-toggle="dropdown">
              <img :src=img_cur />
            </a>
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="languageDropdown">
              <a class="dropdown-item" href="admin.php?lang=eng" target="l">
                <img :src=img_eng alt="English" width="20" class="align-middle mr-1" />
                <span class="align-middle">English</span>
              </a>
              <a class="dropdown-item" href="admin.php?lang=chi" target='l' >
                <img :src="img_chi" alt="Chinese" width="20" class="align-middle mr-1" />
                <span class="align-middle">中文</span>
              </a>
              <a class="dropdown-item" href="admin.php?lang=kor" target='l'>
                <img :src=img_kor alt="Korean" width="20" class="align-middle mr-1" />
                <span class="align-middle">한국어</span>
              </a>
            </div>
          </li>
          <li class="nav-item dropdown">
            <a class="nav-icon dropdown-toggle d-inline-block d-sm-none" href="#" data-toggle="dropdown"><i class="align-middle" data-feather="settings"></i></a>
            <a class="nav-link dropdown-toggle d-none d-sm-inline-block" href="#" data-toggle="dropdown"><i class="align-middle" data-feather="user"></i> <span class="text-dark">Hans Kim</span></a>
            <div class="dropdown-menu dropdown-menu-right">
              <a class="dropdown-item" href="admin.php?fr=profile"><i class="align-middle mr-1" data-feather="user"></i>Profile</a>
              <a class="dropdown-item" href="admin.php" target = "admin_page"><i class="align-middle mr-1" data-feather="pie-chart"></i>Admin Page</a>
              <div class="dropdown-divider"></div>
              <a class="dropdown-item" href="./help.php">Help</a>
              <a class="dropdown-item" href="admin.php?fr=logout">Log Out</a>
            </div>
          </li>
        </ul>
      </div>
    </nav>
</template>
<script setup>
  defineProps({
  img_eng: {
    type: String,
  }
});
  const img_eng = "data:image/png;base64,"
  const img_kor = "data:image/png;base64,";
  const img_chi = "data:image/png;base64,";

  const img_cur = img_eng

</script>