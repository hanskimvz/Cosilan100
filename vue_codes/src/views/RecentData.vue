            ...
            <div class="card">
              <div class="card-body">
                <table class="table table-striped table-bordered table-hover" >
                  <thead>
                    <tr><th> datetime </th>
                      <th v-for="(n, i) in lineChartOptions.series" :key="i">{{n.name}} </th>
                    </tr>
                  </thead>
                  <tbody> 
...
import viewOn      from '@/layout/ViewOn.vue';
...
const query_data = reactive({
  ...
  st:  [], 
  cam: [],
  date_from: getDateString(addDays(new Date(), -7)),
...
const updateData = ( (data)=> {
  ...
  if (key == 'place') {
    ...
    if (data[key]=='7days') {
      query_data.date_from = getDateString(addDays(new Date(), -7));
    }
    else if (data[key]=='4weeks') {
      query_data.date_from = getDateString(addDays(new Date(), -7*4));
    }
    else if (data[key]=='12weeks') {
      query_data.date_from = getDateString(addDays(new Date(), -7*12));
    };
